...
class EventSlicer:
    ...
    def get_events(self, t_start_us: int, t_end_us: int) -> Dict[str, np.ndarray]:
        ...
        t_start_us = max(0, t_start_us - self.t_offset)
        t_end_us = max(0, t_end_us - self.t_offset)
        # t_start_us = max(0, t_start_us)
        # t_end_us = max(0, t_end_us)

        print (f"t_start_us: {t_start_us}, t_end_us: {t_end_us}")
        ...
        events['t'] = (time_array_conservative[idx_start_offset:idx_end_offset] + np.int64(self.t_offset)).astype(np.int64)
